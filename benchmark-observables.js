...
import { makeObservable, observable, autorun, reaction, action, configure } from 'mobx';
...
function benchmarkCreationAndSubscription(iterations = 10000) {
  ...
  // Cami Observable
  results.push(formatResult('Cami Observable', measureTime((i) => {
    const observable = new Observable();
    const subs = observable.subscribe(() => {});
    subs.unsubscribe();
  }, iterations), iterations, 'Observables'));
  
  // RxJS Observable
  results.push(formatResult('RxJS Observable', measureTime((i) => {
  ...
  // Preact Signals
  results.push(formatResult('Preact Signals', measureTime((i) => {
  ...
  results.push(formatResult('Cami ObservableState', measureTime((i) => {
    const state = new ObservableState(0);
    const subs = state.onValue(() => {});
    subs.unsubscribe();
  }, iterations), iterations, 'State Management'));
  
  // MobX
  results.push(formatResult('MobX', measureTime((i) => {
    class Store {
      constructor() {
        this.value = 0;
        makeObservable(this, {
  ...
  // Vue Reactivity
  results.push(formatResult('Vue Reactivity', measureTime((i) => {
  ...
  results.push(formatResult('Cami ObservableStore', measureTime((i) => {
    ...
    subs.unsubscribe();
  }, iterations), iterations, 'Store Libraries'));
  
  // Redux Toolkit
  results.push(formatResult('Redux Toolkit', measureTime((i) => {
  ...
  // MobX-State-Tree
  results.push(formatResult('MobX-State-Tree', measureTime((i) => {
  ...
  // Redux
  results.push(formatResult('Redux', measureTime((i) => {
    ...
    unsubscribe();
  }, iterations), iterations, 'Small Stores'));
  
  // Zustand
  results.push(formatResult('Zustand', measureTime((i) => {
  ...
  // Valtio
  results.push(formatResult('Valtio', measureTime((i) => {
...
function benchmarkValueUpdates(iterations = 10000) {
  ...
  // RxJS Subject
  results.push(formatResult('RxJS Subject', measureTime((i) => {
    const subject = new Subject();
    let called = 0;
...
function benchmarkMultipleSubscribers(iterations = 1000, subscribers = 10) {
  ...
  // CATEGORY: Observables
  // Cami Observable
  results.push(formatResult(`Cami Observable (${subscribers} subscribers)`, measureTime((idx) => {
    const observable = new Observable();
    const subscriptions = [];
  ...
  // RxJS Subject
  results.push(formatResult(`RxJS Subject (${subscribers} subscribers)`, measureTime((idx) => {
    const subject = new Subject();
    const subscriptions = [];
    for (let i = 0; i < subscribers; i++) {
  ...
  // Preact Signals
  results.push(formatResult(`Preact Signals (${subscribers} subscribers)`, measureTime((idx) => {
    const signal = Signals.signal(0);
    const disposers = [];
    for (let i = 0; i < subscribers; i++) {
  ...
  // CATEGORY: State Management
  // Cami ObservableState
  results.push(formatResult(`Cami ObservableState (${subscribers} subscribers)`, measureTime((idx) => {
    const state = new ObservableState(0);
    const subscriptions = [];
    for (let i = 0; i < subscribers; i++) {
  ...
  // MobX
  results.push(formatResult(`MobX (${subscribers} subscribers)`, measureTime((idx) => {
    class Store {
      constructor() {
        this.value = 0;
        makeObservable(this, {
  ...
  // Vue Reactivity
  results.push(formatResult(`Vue Reactivity (${subscribers} subscribers)`, measureTime((idx) => {
    const state = reactive({ value: 0 });
    const stoppers = [];
    for (let i = 0; i < subscribers; i++) {
  ...
  // CATEGORY: Store Libraries
  // Cami ObservableStore
  results.push(formatResult(`Cami ObservableStore (${subscribers} subscribers)`, measureTime((idx) => {
    const testStore = store({
      state: { count: 0 },
  ...
  // Redux Toolkit
  results.push(formatResult(`Redux Toolkit (${subscribers} subscribers)`, measureTime((idx) => {
    const counterSlice = createSlice({
      name: 'counter',
      initialState: { count: 0 },
      reducers: {
  ...
  // MobX-State-Tree
  results.push(formatResult(`MobX-State-Tree (${subscribers} subscribers)`, measureTime((idx) => {
    const CounterModel = types
      .model('Counter', {
        count: types.number
      })
  ...
  // Redux
  results.push(formatResult(`Redux (${subscribers} subscribers)`, measureTime((idx) => {
    const initialState = { count: 0 };
  ...
  // Zustand
  results.push(formatResult(`Zustand (${subscribers} subscribers)`, measureTime((idx) => {
    const useStore = create((set) => ({
      count: 0,
      increment: (value) => set({ count: value })
    }));
  ...
  // Valtio
  results.push(formatResult(`Valtio (${subscribers} subscribers)`, measureTime((idx) => {
    const state = proxy({ count: 0 });
...
function benchmarkDeepObjectUpdates(iterations = 1000) {
  ...
  // CATEGORY: State Management
  // Cami ObservableState
  results.push(formatResult('Cami ObservableState', measureTime((idx) => {
  ...
  // MobX
  results.push(formatResult('MobX', measureTime((idx) => {
    class Store {
      constructor() {
  ...
  // Vue Reactivity
  results.push(formatResult('Vue Reactivity', measureTime((idx) => {
  ...
  // CATEGORY: Store Libraries
  // Cami Store
  results.push(formatResult('Cami Store', measureTime((i) => {
  ...
  // Redux Toolkit
  results.push(formatResult('Redux Toolkit', measureTime((idx) => {
  ...
  // MobX-State-Tree
  results.push(formatResult('MobX-State-Tree', measureTime((idx) => {
  ...
  // Redux
  results.push(formatResult('Redux', measureTime((idx) => {
  ...
  // Zustand
  results.push(formatResult('Zustand', measureTime((idx) => {
  ...
  // Valtio
  results.push(formatResult('Valtio', measureTime((idx) => {
    // Create Valtio store
    const state = proxy({
      user: {
        profile: {
...
function benchmarkComputedValues(iterations = 1000) {
  ...
  results.push(formatResult('Cami ObservableState Computed', measureTime((idx) => {
    const state = new ObservableState(0);
    let value;
  ...
  results.push(formatResult('Preact Signals Computed', measureTime((idx) => {
    const count = Signals.signal(0);
    const doubled = Signals.computed(() => count.value * 2);
    let value;
    
    const dispose = doubled.subscribe(v => { value = v });
  ...
  // CATEGORY: State Management
  // Vue Computed
  results.push(formatResult('Vue Computed', measureTime((idx) => {
  ...
  // MobX Computed
  results.push(formatResult('MobX Computed', measureTime((idx) => {
    class Store {
      constructor() {
        this.count = 0;
        makeObservable(this, {
  ...
  // CATEGORY: Store Libraries
  // Cami Store Memo
  results.push(formatResult('Cami Store Memo', measureTime((idx) => {
  ...
  results.push(formatResult('MobX-State-Tree Computed', measureTime((idx) => {
    const CounterModel = types
      .model('Counter', {
        count: types.number
      })
  ...
  results.push(formatResult('Redux Toolkit with selector', measureTime((idx) => {
    const counterSlice = createSlice({
      name: 'counter',
      initialState: { count: 0 },
      reducers: {
  ...
  // Redux (with selector)
  results.push(formatResult('Redux with selector', measureTime((idx) => {
    const initialState = { count: 0 };
  ...
  results.push(formatResult('Zustand with selector', measureTime((idx) => {
    const useStore = create((set) => ({
      count: 0,
  ...
  results.push(formatResult('Valtio with derived state', measureTime((idx) => {
    const state = proxy({ count: 0 });
...
function benchmarkStoreOperations(iterations = 1000) {
  ...
  results.push(formatResult('Cami Store (Mutation)', measureTime((i) => {
    const uniqueId = `a${i}`;
    const testStore = store({
  ...
  results.push(formatResult('Cami Store (Query)', measureTime((i) => {
    const uniqueId = `d${i}`;
    const testStore = store({
      state: {
        ...createComplexState(),
  ...
  results.push(formatResult('Cami Store (Multiple Actions)', measureTime((i) => {
    const uniqueId = `b${i}`;
    const testStore = store({
      state: createComplexState(),
  ...
  results.push(formatResult('Cami Store (Complex Memo)', measureTime((i) => {
    const uniqueId = `c${i}`;
    const testStore = store({
      state: {
        ...createComplexState(),
  ...
  results.push(formatResult('Redux Toolkit (Mutation)', measureTime((i) => {
    const userSlice = createSlice({
      name: 'user',
  ...
  // Redux Toolkit - Query
  results.push(formatResult('Redux Toolkit (Query)', measureTime((i) => {
    ...
    store.dispatch(apiSlice.actions.fetchSuccess({
      id: 123, 
      name: "API User",
      email: "api@camijs.com"
    }));
  ...
  results.push(formatResult('MobX-State-Tree (Mutation)', measureTime((i) => {
  ...
  results.push(formatResult('MobX-State-Tree (Query)', measureTime((i) => {
  ...
  results.push(formatResult('Redux Toolkit (Multiple Actions)', measureTime((i) => {
    const userSlice = createSlice({
      name: 'user',
      initialState: createComplexState(),
      reducers: {
  ...
  results.push(formatResult('Redux Toolkit (Complex Memo)', measureTime((i) => {
    const userSlice = createSlice({
      name: 'user',
      initialState: {
        ...createComplexState(),
  ...
  results.push(formatResult('MobX-State-Tree (Multiple Actions)', measureTime((i) => {
    const NotificationsModel = types
      .model('Notifications', {
        email: types.boolean,
        push: types.boolean,
  ...
  results.push(formatResult('MobX-State-Tree (Complex Memo)', measureTime((i) => {
    const NotificationsModel = types
      .model('Notifications', {
        email: types.boolean,
        push: types.boolean,
      ...
      .views(self => ({
        get userStats() {
          return {
            ...
            totalTags: self.posts.reduce((acc, post) => acc.tags?.length ? acc + post.tags.length : acc, 0),
            preferences: {
...
function benchmarkUrlStoreIntegration(iterations = 500) {
  ...
  results.push(formatResult('URL Store Integration', measureTime((i) => {
