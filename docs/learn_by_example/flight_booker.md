# Flight Booker

<div class="cami-live-example">
  <div class="cami-live-example__header"><span class="cami-live-example__label">Live island</span><span class="cami-live-example__note">Runs locally in this page</span></div>
  <div class="cami-live-example__stage"><cami-demo-island kind="flight"></cami-demo-island></div>
</div>

## Page shell

```html
<flight-booker></flight-booker>

<script src="./build/cami.cdn.js"></script>
<!-- CDN version below -->
<!-- <script src="https://unpkg.com/cami@0.4.1/build/cami.cdn.js"></script> -->
<script type="module" src="./island.js"></script>

```

## Island source

<!-- cami-language-pair -->
=== "JavaScript"

    ```javascript
    --8<-- "docs/examples/islands/flight_booker.js"
    ```

=== "TypeScript"

    ```typescript
    --8<-- "docs/examples/islands/flight_booker.ts"
    ```
