...
\f\r"'\`<>=]`;
  ...
  var _DependencyTracker = class _DependencyTracker {
    ...
    addDependency(store2, property) {
      ...
      this.dependencies.add({ store: store2, property });
    }
  };
    ...
    fetch(queryName, ...args) {
      ...
      return this._fetchWithRetry(queryFn, args, retry, retryDelay).then((data) => {
        ...
        if (onSuccess) {
          ...
          onSuccess(__spreadValues({ result: resultData }, context));
        }
        return data;
      }).catch((error) => {
        resultError = error;
        if (onError) {
          ...
          onError(__spreadValues({ result: resultError }, context));
        }
        throw error;
      }).finally(() => {
        if (onSettled) {
          __trace(`fetch`, `Fetch settled: ${queryName}`);
          onSettled(__spreadValues({ result: resultData, error: resultError }, context));
        }
      });
    }
    /**
    ...
    mutate(mutationName, ...args) {
      ...
      return mutationFn(...args).then((data) => {
        ...
        if (onSettled) {
          ...
          onSettled(__spreadValues({ result: resultData || resultError }, context));
        }
      });
    }
  };
  var slice = (store2, { name, state, actions, queries, mutations }) => {
    ...
    const _storedState = JSON.parse(localStorage.getItem(`${store2.name}`));
    ...
    store2.state[name] = initialState;
    const sliceActions = {};
    const sliceQueries = {};
    ...
    const sliceState = new Proxy(store2.state[name], {
      ...
      set(target, property, value) {
        ...
        if (store2.devTools) {
          store2.devTools.send(`${name}.${property}`, store2.state);
        }
        return true;
      }
    });
    Object.keys(actions).forEach((actionKey) => {
      ...
      store2.action(namespacedAction, ({ state: state2, payload }) => {
        state2[name] = produce(state2[name], (draft) => {
    ...
    if (mutations) {
      Object.keys(mutations).forEach((mutationKey) => {
        ...
        const mutationConfig = __spreadProps(__spreadValues({}, mutations[mutationKey]), { actions: sliceActions, queries: sliceQueries, invalidateQueries: store2.invalidateQueries.bind(store2) });
        store2.mutation(namespacedMutation, mutationConfig);
        sliceMutations[mutationKey] = (...args) => {
          return store2.mutate(namespacedMutation, ...args);
        };
      });
    }
  ...
  var _localStorageEnhancer = (StoreClass) => {
    return (initialState, options) => {
      ...
      store2.reset = () => {
        localStorage.removeItem(storeName);
        localStorage.removeItem(`${storeName}-expiry`);
        store2.state = store2._createProxy(initialState);
