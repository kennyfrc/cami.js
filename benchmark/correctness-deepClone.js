...
const testCases = [
  ...
  { name: 'Simple object', input: { a: 1, b: 2, c: 'hello' }, shouldBeCloned: true },
  { name: 'Nested object', input: { a: { b: { c: 1 } } }, shouldBeCloned: true },
  
  // Arrays (should be cloned)
  { name: 'Empty array', input: [], shouldBeCloned: true },
  { name: 'Simple array', input: [1, 2, 3], shouldBeCloned: true },
  { name: 'Nested array', input: [[1, 2], [3, 4]], shouldBeCloned: true },
  { name: 'Mixed array', input: [1, 'hello', { a: 1 }, [2, 3]], shouldBeCloned: true },
  
  // Dates (should be cloned)
  { name: 'Date object', input: new Date('2023-01-01'), shouldBeCloned: true },
  ...
  { name: 'Simple map', input: new Map([['a', 1], ['b', 2]]), shouldBeCloned: true },
  { name: 'Nested map', input: new Map([['obj', { a: 1 }], ['arr', [1, 2, 3]]]), shouldBeCloned: true },
  ...
  { name: 'Set with objects', input: new Set([{ a: 1 }, { b: 2 }]), shouldBeCloned: true },
  
  // TypedArrays (should be cloned)
  { name: 'Int8Array', input: new Int8Array([1, 2, 3]), shouldBeCloned: true },
  { name: 'Uint8Array', input: new Uint8Array([1, 2, 3]), shouldBeCloned: true },
  { name: 'Float32Array', input: new Float32Array([1.1, 2.2, 3.3]), shouldBeCloned: true },
  { name: 'Float64Array', input: new Float64Array([1.1, 2.2, 3.3]), shouldBeCloned: true },
  ...
  try {
    ...
    if (original !== null && typeof original === 'object') {
      if (original.constructor !== cloned.constructor) {
        errors.push(`Constructor mismatch: ${original.constructor.name} !== ${cloned.constructor.name}`);
      }
      ...
      if (original instanceof RegExp) {
        if (!(cloned instanceof RegExp) || original.source !== cloned.source || original.flags !== cloned.flags) {
    ...
    return { success: false, errors: [`Exception: ${error.message}`], result: null };
  }
}
