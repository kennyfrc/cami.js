  ...
  if (a instanceof Set) {
    ...
    for (let i = 0; i < aValues.length; i++) {
      ...
      for (let j = 0; j < bValues.length; j++) {
        if (
          !matched[j] &&
          _deepEqualCircular(aValues[i], bValues[j], visited)
        ) {
          matched[j] = true;
          found = true;
          break;
  ...
  function merge(target: any, source: any): any {
    ...
    if (source instanceof Map) {
      ...
      for (const [key, val] of source.entries()) {
        const keyClone =
          key === null || typeof key !== "object" ? key : merge(undefined, key);
...
const _deepClone = (
  ...
  if (value instanceof Map) {
    ...
    for (const [key, val] of value.entries()) {
      ...
      const keyClone =
        key === null || typeof key !== "object" ? key : _deepClone(key, cache);

      const valClone =
        val === null || typeof val !== "object" ? val : _deepClone(val, cache);
