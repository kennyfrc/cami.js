...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var http = (config2) => {
    ...
    return new HTTPStream((observer) => {
      ...
      xhr.open(config2.method || "GET", config2.url);
      if (config2.headers) {
      ...
      xhr.onload = () => {
        ...
        const transformResponse = config2.transformResponse || ((data) => {
          try {
      ...
      xhr.send(config2.data ? JSON.stringify(config2.data) : null);
      return () => {
        xhr.abort();
      };
    });
  };
  http.get = (url, config2 = {}) => {
    config2.url = url;
    config2.method = "GET";
    return http(config2);
  };
  http.post = (url, data = {}, config2 = {}) => {
    config2.url = url;
    config2.data = data;
    config2.method = "POST";
    return http(config2);
  };
  http.put = (url, data = {}, config2 = {}) => {
    config2.url = url;
    config2.data = data;
    config2.method = "PUT";
    return http(config2);
  };
  http.patch = (url, data = {}, config2 = {}) => {
    config2.url = url;
    config2.data = data;
    config2.method = "PATCH";
    return http(config2);
  };
  http.delete = (url, config2 = {}) => {
    config2.url = url;
    config2.method = "DELETE";
    return http(config2);
  };
  http.sse = (url, config2 = {}) => {
    const stream = new HTTPStream((observer) => {
      const source = new EventSource(url, config2);
  ...
  var ReactiveElement = class extends HTMLElement {
     ...
     */
    setup(config2) {
      if (config2.infer === true) {
        config2.infer = ["observables", "computed"];
      }
      if (config2.infer) {
        config2.infer.forEach((type) => {
          if (type === "observables") {
            ...
            Object.getOwnPropertyNames(Object.getPrototypeOf(this)).filter((key) => {
      ...
      if (config2.observables) {
        config2.observables.forEach((key) => {
          if (this[key] instanceof Observable) {
            return;
          }
          const observable = this.observable(this[key], key);
      ...
      if (config2.effects) {
        config2.effects.forEach((effectFn) => {
          this.effect(effectFn);
        });
      }
      if (config2.attributes) {
        Object.entries(config2.attributes).forEach(([attr, parseFn]) => {
          const observableAttr = typeof parseFn === "function" ? this.observableAttr(attr, parseFn) : this.observableAttr(attr);
    ...
    observable(initialValue, name = null) {
      ...
      const observable = new ObservableState(initialValue, (value) => this.react.bind(this)(), { last: true, name });
    ...
    query({ queryKey, queryFn, staleTime = 0, refetchOnWindowFocus = true, refetchOnMount = true, refetchOnReconnect = true, refetchInterval = null, gcTime = 1e3 * 60 * 5, retry = 3, retryDelay = (attempt) => Math.pow(2, attempt) * 1e3 }) {
      console.log("queryKey type:", typeof queryKey);
      console.log("queryKey value:", queryKey);
      const key = Array.isArray(queryKey) ? queryKey.join(":") : queryKey;
      this.queryFunctions.set(key, queryFn);
     ...
     */
    refetchQuery(key) {
      const queryFn = this.queryFunctions.get(key);
      if (queryFn) {
    ...
    connect(store2, key) {
      ...
      const observable = this.observable(store2.state[key], key);
