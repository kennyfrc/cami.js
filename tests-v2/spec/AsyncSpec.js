...
describe("Async Actions - Advanced Scenarios", () => {
  ...
  it("should handle asynchronous API requests", async () => {
    const mockApi = {
      ...
      fetchPosts: (userId) =>
        Promise.resolve([{ id: 1, title: `Post by User ${userId}` }]),
    };

    userStore.defineAsyncAction(
      "fetchUserAndPosts",
      async ({ dispatch }, userId) => {
        dispatch("setLoading", true);
        try {
  ...
  it("should handle complex sequences of actions", async () => {
    const mockApi = {
      ...
      fetchUserPosts: (userId) =>
        Promise.resolve([{ id: 1, title: "First Post" }]),
    };
    ...
    const result = await userStore.dispatchAsync(
      "loginAndFetchUserData",
      mockToken
    );
