...
describe("Hooks", function () {
  ...
  beforeEach(function () {
    appStore = store({
      state: {
        ...
        subscription: { id: "sub1", teamId: "team1", plan: "basic", seats: 5 },
        team: {
          id: "team1",
          name: "My Team",
          ownerId: "user1",
  ...
  it("should use beforeHook for input validation", function () {
    ...
    expect(() =>
      appStore.dispatch("incrementCount", "not a number")
    ).toThrowError("Payload must be a number for incrementCount action");
  ...
  it("should apply both beforeHook and afterHook", function () {
    ...
    expect(() => appStore.dispatch("incrementCount", -1)).toThrowError(
      "Cannot decrement count"
    );
  ...
  it("should roll back to previous state if afterHook throws an error", function () {
    ...
    expect(() => appStore.dispatch("incrementCount", 10)).toThrowError(
      "Count cannot exceed 10"
    );
  ...
  it("should maintain consistent state across multiple actions if hook throws an error", function () {
    ...
    expect(() => appStore.dispatch("incrementCount", 5)).toThrowError(
      "Count cannot exceed 10"
    );
  ...
  it("should validate complex object structures", function () {
    const schema = {
      user: Type.Product({
        ...
        email: Type.Refinement(Type.String, (s) =>
          /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(s)
        ),
      }),
    };
    ...
    ).toThrowError(/Refinement predicate failed at user.age/);
    expect(() =>
      appStore.dispatch("setUser", {
        name: "Bob",
        age: 40,
    ...
    ).toThrowError(/Refinement predicate failed at user.email/);
  });
  ...
  it("should validate array types", function () {
    ...
    expect(() => appStore.dispatch("addItem", "15")).toThrowError(
      /Expected float, got string at items.2/
    );
  ...
  it("should handle dependent types", function () {
    ...
    expect(() =>
      appStore.dispatch("updateSettings", { theme: "default", fontSize: 12 })
    ).not.toThrow();
    expect(() =>
      appStore.dispatch("updateSettings", {
        theme: "large-print",
        fontSize: 18,
      })
    ).not.toThrow();
    ...
    ).toThrowError(/Font size must be at least 16 for large-print theme/);
  });
  ...
  it("should handle complex dependent types with team, subscription, and user interrelations", function () {
    ...
    expect(() =>
      appStore.dispatch("updateSettings", { theme: "default", fontSize: 16 })
    ).not.toThrow();

    expect(() =>
      appStore.dispatch("updateSettings", { theme: "dark", fontSize: 16 })
    ).toThrowError(
      /Custom themes are only available for premium subscriptions/
    );
    ...
    const schema = {
      user: Type.DependentRecord(
        ...
        (fields) => {
          ...
          const invalidPermissions = fields.permissions.filter(
            (p) => !allowedPermissions.includes(p)
          );
          if (invalidPermissions.length > 0) {
            return `Invalid permissions for role ${
              fields.role
