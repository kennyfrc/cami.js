...
class TaskManagerElement extends ReactiveElement {
  ...
  template() {
    return html`
            ...
            <button
                @click=${() => this.addTask(document.getElementById('taskInput').value)}
            >
                Add Task
            </button>
            <button @click=${() => this.removeFirstTask()}>
                Remove First Task
            </button>
            <button @click=${() => this.removeLastTask()}>
                Remove Last Task
            </button>
            <button
                @click=${() => this.addTaskToFront(document.getElementById('taskInput').value)}
            >
                Add Task to Front
            </button>
            <button
                @click=${() => this.replaceTask(1, document.getElementById('taskInput').value)}
            >
                Replace Second Task
            </button>
            ...
            <button
                @click=${() => this.fillTasks(document.getElementById('taskInput').value)}
            >
                Fill Tasks
            </button>
            <button @click=${() => this.copyWithinTasks(0, 1, 2)}>
                Copy Within Tasks by Index from 0 to 1
            </button>
            <ul>
                ${this.tasks.map(
                  (task, index) => html`
