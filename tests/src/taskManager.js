...
class TaskManagerElement extends ReactiveElement {
  ...
  template() {
    return html`
      ...
      <button @click=${() => this.addTask(document.getElementById('taskInput').value)}>
        Add Task
      </button>
      <button @click=${() => this.removeFirstTask()}>Remove First Task</button>
      <button @click=${() => this.removeLastTask()}>Remove Last Task</button>
      <button @click=${() => this.addTaskToFront(document.getElementById('taskInput').value)}>
        Add Task to Front
      </button>
      <button @click=${() => this.replaceTask(1, document.getElementById('taskInput').value)}>
        Replace Second Task
      </button>
      <button @click=${() => this.sortTasks()}>Sort Tasks</button>
      <button @click=${() => this.reverseTasks()}>Reverse Tasks</button>
      <button @click=${() => this.fillTasks(document.getElementById('taskInput').value)}>
        Fill Tasks
      </button>
